...
const FLUTE_CONST = {
  ...
  // Breath hole (Flutopedia "Breath Hole-Width" / "Breath Hole-Length"):
  // the passage the player blows through into the SAC at the mouth end.
  // No published formula ties it to bore; these defaults land in the
  // common hand-made range (~1/4"–3/8" wide, ~1/2"–3/4" passage) and
  // scale gently with bore.
  breathHoleWidth(bore) { return Math.max(0.19, Math.min(bore * 0.45, 0.38)); },
  breathHoleLength(bore) { return Math.max(0.40, Math.min(bore * 0.85, 0.80)); },
...
function saveNestToLibrary(name, nest) {
  ...
  const entry = {
    ...
    rampCurve: Number.isFinite(nest.rampCurve) ? nest.rampCurve : 0,
    tshWidthIn: Number.isFinite(nest.tshWidthIn) ? nest.tshWidthIn : null,
    wallThicknessIn: Number.isFinite(nest.wallThicknessIn) ? nest.wallThicknessIn : null,
    backsetIn: Number.isFinite(nest.backsetIn) ? nest.backsetIn : 0,
    tipFlatIn: Number.isFinite(nest.tipFlatIn) ? nest.tipFlatIn : 0.01,
    chimneyIn: Number.isFinite(nest.chimneyIn) ? nest.chimneyIn : 0,
    tipHeightIn: Number.isFinite(nest.tipHeightIn) ? nest.tipHeightIn : 0.0078,
    flueLengthIn: Number.isFinite(nest.flueLengthIn) ? nest.flueLengthIn : null,
  };
  const updated = [entry, ...items];
...
function parseNestFile(obj) {
  ...
  return {
    ...
    rampCurve: num(obj.rampCurve), // optional; older files simply lack it
    tshWidthIn: num(obj.tshWidthIn), // optional (added with the TSH-width control)
    wallThicknessIn: num(obj.wallThicknessIn),
    backsetIn: num(obj.backsetIn),
    tipFlatIn: num(obj.tipFlatIn),
    chimneyIn: num(obj.chimneyIn),
    tipHeightIn: num(obj.tipHeightIn),
    flueLengthIn: num(obj.flueLengthIn),
  };
}
      ...
      {chambers.map((c, idx) => {
        ...
        return (
          <g key={idx}>
            ...
            {/* Breath hole at the mouth end (Flutopedia "Breath Hole") —
                drawn at true scale on the mouthpiece stub */}
            {Number.isFinite(c.breathHoleWidthIn) && (() => {
              const bx = sacX0 + 9;
              const br = Math.min(TH * 0.35, Math.max(2.5, (c.breathHoleWidthIn * sc) / 2));
              return (
                <g>
                  <circle cx={bx} cy={cy(bx)} r={br} fill="#0c0600" stroke="#7dd3fc" strokeWidth="1.5"/>
                  <text x={bx} y={cy(bx) + TH/2 + 12} textAnchor="middle" fill="#7dd3fc" fontSize="8" fontFamily="system-ui">
                    BREATH {fmt(c.breathHoleWidthIn, 2)}"
                  </text>
                </g>
              );
            })()}
...
function buildBirdBlock(birdKey, melodyChamber, curve, heightFraction = 1) {
  ...
  // Base footprint: match the DEFAULT bird exactly — it spans the flue
  // (2× TSH width) plus the SAC exit opening (one TSH length) plus the
  // small ceiling overlap, ending right AT the TSH opening (never over
  // it). Width matches the Default bird's rail width.
  const shWEff = parseFloat(melodyChamber.shW) || FLUTE_CONST.soundHoleWidth(bore);
  const shLEff = parseFloat(melodyChamber.shL) || FLUTE_CONST.soundHoleLength(bore);
  const targetWidthIn = Math.min(shWEff * 1.5, bore);
  const targetLengthIn = 2 * shWEff + shLEff + 0.12;
  ...
  const wallTEff = (Number.isFinite(melodyChamber.nestWallThicknessIn) && melodyChamber.nestWallThicknessIn > 0)
    ? Math.max(0.04, Math.min(melodyChamber.nestWallThicknessIn, 0.5))
    : Math.max(0.05, (bore / 2) * 0.28);
  const outerR = bore / 2 + wallTEff;
  ...
  // Position like the DEFAULT bird: the base's DOWNSTREAM edge lands
  // exactly at the TSH window's upstream edge (the nest's x = 0 is at
  // tTSH), so the block covers flue + exit opening and stops at the TSH.
  const worldPos = tshCenter.clone()
    .sub(forward.clone().multiplyScalar(targetLengthIn / 2))
    .add(up.clone().multiplyScalar(liftDistance));
...
function buildChamberMesh(c, curve, materialColor, holeShape = "round", surfaceRoughness = 0.75, surfaceMetalness = 0.05, birdKey = "none") {
  const group = new THREE.Group();
  ...
  const r = c.bore / 2;
  // Outer wall thickness: user-adjustable (Prairie: 1/8–3/16 bore wall,
  // 3/16–1/4 top wall); falls back to the bore-scaled heuristic when unset.
  const wallT = (Number.isFinite(c.nestWallThicknessIn) && c.nestWallThicknessIn > 0)
    ? Math.max(0.04, Math.min(c.nestWallThicknessIn, 0.5))
    : Math.max(0.05, r * 0.28);
  ...
  let nestBlockGeo = null;       // the internal block (plug with ramp face + flue-floor top), unioned into the body LAST
  let nestBirdMesh = null;       // the functional bird (flue roof), a separate mesh added after the body
  {
    ...
    if (c.bore <= FLUE_CHANNEL_BORE_LIMIT) {
      ...
      const flueLength = (Number.isFinite(c.nestFlueLengthIn) && c.nestFlueLengthIn > 0)
        ? Math.max(0.1, Math.min(c.nestFlueLengthIn, 2))
        : 2 * shW; // Prairie default: flue length ≈ 2× flue width; user-adjustable independently of width
      ...
      const fippleAngleDeg = (Number.isFinite(c.nestFippleAngleDeg) && c.nestFippleAngleDeg > 0)
        ? c.nestFippleAngleDeg : 35; // Flow Studio's default cutting-edge bevel

      // ═══ NEST — the exact Flow Studio wind-chamber profile ═══════════
      // (x = 0 at the TSH window start, y = 0 at the outer surface apex;
      // air flows left → right, all dimensions straight from the design):
      //   SAC ceiling (tube wall, intact) → EXIT OPENING one TSH-length
      //   long → BLOCK: base constrained at the bore bottom, top = the
      //   flue floor (surface − flue depth), upstream face = the RAMP
      //   ascending at the design angle, downstream face at the TSH →
      //   FLUE of the selected depth carved over the block, length 2×TSH-
      //   width → TSH window → CUTTING EDGE: tip just below jet level,
      //   fipple-angle bevel dropping into the bore-side wall, ~15° outer
      //   relief above. The bird (added separately below) roofs the flue
      //   and covers the exit opening, its right edge at the TSH.
      ...
      const rampRad = Math.max(0.1, rampAngleDegEffective * Math.PI / 180);

      const xTsh0 = 0, xTsh1 = shL;          // TSH window span
      const xFlue0 = -flueLength;             // flue entrance = block leading edge
      const xExit1 = xFlue0;                  // exit opening ends at the flue entrance
      const xExit0 = xExit1 - shL;            // SAC ceiling stops ONE TSH-LENGTH before the bird

      // Cutting edge, same construction as the Flow Studio viewer:
      // Fipple tip height: sits ABOVE the bottom of the flue channel by
      // 1/128" (Prairie), user-adjustable; clamped just under the flue roof.
      ...
      // ── Air-space subtraction: ONE full-depth region from the exit
      // opening's upstream edge through the cutting edge. The block union
      // (performed LAST — subtract-then-union is the safe order for this
      // CSG library, see the note at the union below) restores the ramp
      // and the flue floor inside this void, leaving exactly the Flow
      // Studio air path.
      const ceilRun = tshCutDepth / Math.tan(rampRad); // SAC-ceiling end chamfer run
      const shape = new THREE.Shape();
      shape.moveTo(xExit0, 0.15);
      ...
      channelGeo.translate(0, 0, -shW / 2); // center the cut across the flue's width
      ...
      // ── BLOCK: built now (same local frame), unioned into the body at
      // the very end. Base at the bore bottom (constrained), top at the
      // flue floor — a deeper flue lowers the block's top, never the bird.
      const yBot = -(wallT + 2 * r + 0.02);        // just past the bore bottom, guaranteeing a clean merge
      const rampRise = (-flueDepth) - yBot;
      const rampRun = Math.min(rampRise / Math.tan(rampRad), Math.max(0.1, c.sacLen * 0.7));
      const xRampBase = xExit1 - rampRun;
      const rampCurveK = Math.max(0, Math.min(1, Number.isFinite(c.nestRampCurve) ? c.nestRampCurve : 0));
      const blockShape = new THREE.Shape();
      blockShape.moveTo(xRampBase, yBot);
      if (rampCurveK > 0.01) {
        // Curved ramp: quadratic bezier sagging below the straight chord
        // (control point pulled toward the base-side corner) — a concave,
        // scooped entry. 0 = the straight face; 1 = full curve.
        const mx = (xRampBase + xExit1) / 2, my = (yBot + (-flueDepth)) / 2;
        blockShape.quadraticCurveTo(mx + rampCurveK * (xExit1 - mx), my + rampCurveK * (yBot - my), xExit1, -flueDepth);
      } else {
        blockShape.lineTo(xExit1, -flueDepth);     // the RAMP face, flush at the bird's leading edge
      }
      blockShape.lineTo(xTsh0 - backset, -flueDepth); // block top = flue floor
      blockShape.lineTo(xTsh0 - backset, yBot);       // downstream face, set back by the BACKSET
      blockShape.closePath();
      const blockW = 2 * (r + wallT) + 0.1;
      const blockGeo = new THREE.ExtrudeGeometry(blockShape, { depth: blockW, bevelEnabled: false, steps: 1 });
      blockGeo.translate(0, 0, -blockW / 2);
      blockGeo.applyMatrix4(basis);
      blockGeo.translate(originPoint.x, originPoint.y, originPoint.z);
      // Clip to the tube so the full-width block never pokes out of the
      // round surface (the flue cut above is only shW wide; outside that
      // width the block merges invisibly into the wall).
      const clipGeo = buildCappedTubeGeometry(curvePath, r + wallT, segments, 20);
      {
        const blockBrush0 = new Brush(blockGeo); blockBrush0.updateMatrixWorld();
        const clipBrush = new Brush(clipGeo); clipBrush.updateMatrixWorld();
        nestBlockGeo = evaluator.evaluate(blockBrush0, clipBrush, INTERSECTION).geometry;
      }

      // ── BIRD ("Default" in the Bird/Totem picker): the functional roof
      // of the flue. Right edge exactly at the TSH opening; extends
      // upstream over the flue AND the exit opening. Underside CSG-fitted
      // to the round tube. Built only when the "Default" bird is
      // selected — "None" leaves the flute bare, and the decorative STL
      // birds remain independent options.
      if (birdKey === "default") {
        const chimney = Math.max(0, Math.min(Number.isFinite(c.nestChimneyIn) ? c.nestChimneyIn : 0, 0.5));
        const xBird0 = xExit0 - 0.12;
        const birdH = Math.max(0.18, wallT + 0.12);
        const birdW = Math.min(shW * 1.5, 2 * r);
        let birdBox = new THREE.BoxGeometry(-xBird0, birdH + 0.15, birdW);
        birdBox.translate(xBird0 / 2, (birdH - 0.15) / 2, 0); // span x ∈ [xBird0, 0], sink 0.15 below apex for a gapless fit
        if (chimney > 0.005) {
        ...
        birdBox.applyMatrix4(basis);
        birdBox.translate(originPoint.x, originPoint.y, originPoint.z);
        const birdBrush = new Brush(birdBox); birdBrush.updateMatrixWorld();
        ...
        nestBirdMesh.name = "nestBird";
      }

      // Tracking points for the 3D viewer's on-canvas dimension labels —
      // same (basis, originPoint) transform as the geometry, so labels
      // line up exactly (converted to world by the caller via
  ...
  if (c.playable) {
    c.holes.forEach(h => {
      ...
      const holeGeo = (() => {
        // Shape-aware cutter, honoring the selected finger-hole shape with
        // the same semantics HOLE_SHAPES documents (and the same numbers
        // the results table reports via holeShapeDiameter):
        //   round       — straight cylinder at the listed diameter
        //   oval        — listed diameter is the MAJOR axis, elongated
        //                 along the tube (ratio ~1.4:1 per the howTo)
        //   undercut    — surface opening at the listed diameter, interior
        //                 (bore-side) edge beveled ~35% wider (frustum)
        //   countersunk — round hole; outer rim chamfer added separately
        if (holeShape === "oval") {
          const g = new THREE.CylinderGeometry(holeR, holeR, cutDepth, 32);
          g.scale(1, 1, 1 / 1.4); // local Z = across the tube → minor axis
          return g;
        }
        if (holeShape === "undercut") {
          // CylinderGeometry(top, bottom): local +Y = outward → top is the
          // surface opening, bottom is the flared bore-side edge.
          return new THREE.CylinderGeometry(holeR, holeR * 1.35, cutDepth, 32);
        }
        return new THREE.CylinderGeometry(holeR, holeR, cutDepth, 32);
      })();
      // Orient with a full basis (not just an up-quaternion): local Y = cut
      // axis (outward), local X = along the tube — so the oval's major axis
      // runs down the tube's length, exactly as the shape guide describes.
      const tangent = curvePath.getTangentAt(Math.min(0.999, Math.max(0.001, t))).normalize();
      const xAxis = tangent.clone().sub(up.clone().multiplyScalar(tangent.dot(up)));
      if (xAxis.lengthSq() < 1e-8) xAxis.set(1, 0, 0);
      xAxis.normalize();
      const zAxis = new THREE.Vector3().crossVectors(xAxis, up).normalize();
      const basis = new THREE.Matrix4().makeBasis(xAxis, up.clone(), zAxis);
      holeGeo.applyMatrix4(basis);
      const cutCenter = center.clone().add(up.clone().multiplyScalar(cutCenterDist));
      ...
      if (holeShape === "countersunk") {
        // Outer-rim chamfer only: a shallow cone eating no more than ~15%
        // of the wall thickness (HOLE_SHAPES howTo), rim widened to 1.5×
        // AT the surface plane. The frustum extends slightly above the
        // surface (cutting air) so the rim edge is clean; radii are
        // interpolated so the 1.5× width lands exactly at the surface.
        const chamferDepth = Math.max(0.02, Math.min(wallT * 0.3, holeR * 0.8));
        const above = 0.03;
        const coneH = chamferDepth + above;
        const slope = (0.5 * holeR) / chamferDepth;              // radius growth per unit height
        const topR = holeR * 1.5 + slope * above;                 // radius at surface + above
        const coneGeo = new THREE.CylinderGeometry(topR, holeR, coneH, 32);
        coneGeo.applyMatrix4(basis);
        const chamferCenter = center.clone().add(up.clone().multiplyScalar(outerR - chamferDepth + coneH / 2));
        coneGeo.translate(chamferCenter.x, chamferCenter.y, chamferCenter.z);
        const coneBrush = new Brush(coneGeo); coneBrush.updateMatrixWorld();
        const curBrush2 = new Brush(bodyResult.geometry); curBrush2.updateMatrixWorld();
        bodyResult = evaluator.evaluate(curBrush2, coneBrush, SUBTRACTION);
      }
    });
  }
  ...
  {
    if (nestBlockGeo) {
      // ── BLOCK union — the internal plug per the Flow Studio nest
      // profile, built earlier in the nest section: base at the bore
      // bottom, top = the flue floor, upstream face = the ascending ramp.
      // The real air path is over the block's top through the flue (no
      // drilled exit port needed — the exit OPENING through the wall was
      // already cut as part of the nest's air-space subtraction). Union
      // LAST: subtract-then-union is the verified-safe operation order
      // for this CSG library (union-then-subtract at the same location
      // silently corrupts the result — see the raycasting-verified note
      // in this file's history).
      const blockBrush = new Brush(nestBlockGeo); blockBrush.updateMatrixWorld();
      const cur = new Brush(bodyResult.geometry); cur.updateMatrixWorld();
      bodyResult = evaluator.evaluate(cur, blockBrush, ADDITION);
    } else {
      // Large-bore fallback (no flue-channel nest): the original thin
      // internal wall + small drilled SAC exit port.
      ...
      const exitDepth = wallThickness + 0.03;
  ...
  if (nestBirdMesh) group.add(nestBirdMesh);
  ...
  // Mouth end: a real mouthpiece plug with the BREATH HOLE bored through
  // it (Flutopedia "Breath Hole-Width" / "Breath Hole-Length") — the
  // passage the player blows through into the SAC. The old solid cap
  // (with a comment claiming NAFs are sealed here) was simply wrong: this
  // app's own dimension references show the breath hole explicitly.
  {
    const bhW = FLUTE_CONST.breathHoleWidth(c.bore);
    const bhL = FLUTE_CONST.breathHoleLength(c.bore);
    const tangent0 = curvePath.getTangentAt(0.001).normalize();
    const plugGeo = new THREE.CylinderGeometry(r + wallT, r + wallT, bhL, 20);
    const holeGeo0 = new THREE.CylinderGeometry(bhW / 2, bhW / 2, bhL + 0.1, 20);
    const q0 = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 1, 0), tangent0);
    plugGeo.applyQuaternion(q0); holeGeo0.applyQuaternion(q0);
    const plugCenter = centerAt(0).add(tangent0.clone().multiplyScalar(bhL / 2));
    plugGeo.translate(plugCenter.x, plugCenter.y, plugCenter.z);
    holeGeo0.translate(plugCenter.x, plugCenter.y, plugCenter.z);
    const pB = new Brush(plugGeo); pB.updateMatrixWorld();
    const hB = new Brush(holeGeo0); hB.updateMatrixWorld();
    const plugResult = evaluator.evaluate(pB, hB, SUBTRACTION);
    plugResult.geometry.computeVertexNormals();
    const plugMesh = new THREE.Mesh(plugResult.geometry, new THREE.MeshStandardMaterial({ color: "#2a1c10", roughness: 0.85 }));
    plugMesh.name = "breathPlug";
    group.add(plugMesh);
  }
  ...
  useEffect(() => {
    ...
    const buildTimer = setTimeout(() => {
      ...
      try {
        ...
        // Chamber spacing: "separate" keeps distinct pipes with daylight
        // between them; "solid" fuses adjacent pipes wall-to-wall (with a
        // slight overlap so the walls merge) as one milled body.
        const outerRFor = (cc) => cc.bore / 2 + (
          (Number.isFinite(cc.nestWallThicknessIn) && cc.nestWallThicknessIn > 0)
            ? Math.max(0.04, Math.min(cc.nestWallThicknessIn, 0.5))
            : Math.max(0.05, (cc.bore / 2) * 0.28));
        const sepStep = Math.max(...chambers.map(cc => cc.bore)) * 2.6;
        const zPositions = [];
        let zCursor = 0;
        chambers.forEach((cc, idx) => {
          if (idx > 0) zCursor += droneBody === "solid"
            ? (outerRFor(chambers[idx - 1]) + outerRFor(cc)) * 0.96
            : sepStep;
          zPositions.push(zCursor);
        });
        const chamberLens = [];
        chambers.forEach((c, i) => {
          ...
          const { group, totalLen, nestTrackingPoints } = buildChamberMesh(c, curve, materialColor, holeShape, surfaceRoughness, surfaceMetalness, birdKey);
          ...
          rootGroup.add(group);
          chamberLens.push(totalLen);
          maxLen = Math.max(maxLen, totalLen);
        });
        // "One solid" on a straight body: fill the tangent notch between
        // adjacent pipes with a connecting web so the assembly reads as a
        // single block, not two touching tubes.
        if (droneBody === "solid" && chambers.length > 1 && curveBowAmplitudeIn(curve) === 0) {
          for (let i = 0; i < chambers.length - 1; i++) {
            const lenMin = Math.min(chamberLens[i], chamberLens[i + 1]);
            const rMin = Math.min(outerRFor(chambers[i]), outerRFor(chambers[i + 1]));
            const webGeo = new THREE.BoxGeometry(lenMin * 0.96, rMin * 1.15, zPositions[i + 1] - zPositions[i]);
            webGeo.translate((lenMin * 0.96) / 2, 0, (zPositions[i] + zPositions[i + 1]) / 2);
            const web = new THREE.Mesh(webGeo, new THREE.MeshStandardMaterial({
              color: materialColor, roughness: surfaceRoughness, metalness: surfaceMetalness }));
            web.castShadow = true; web.receiveShadow = true;
            rootGroup.add(web);
          }
        }
        ...
        scene.add(rootGroup);
        // Debug/verification handle (console + tests): raycasts against this
        // scene must account for rootGroup's centering X-offset.
        try {
          window.__flute3d = {
            scene, rootGroup,
            makeRay: () => new THREE.Raycaster(),
...
function FlutePage({ loadConfig, onConfigLoaded }) {
  ...
  const [droneBody,      setDroneBody]      = useState("separate"); // "separate" pipes vs "solid" one-piece body
  ...
  useEffect(() => {
    ...
    if (c.bore !== undefined) setBore(Math.min(2, c.bore)); // bore capped at 2"
    ...
    if (c.droneBody !== undefined) setDroneBody(c.droneBody);
    ...
    setNestTipHeightIn(c.nestTipHeightIn !== undefined ? c.nestTipHeightIn : null);
    setNestFlueLengthIn(c.nestFlueLengthIn !== undefined ? c.nestFlueLengthIn : null);
  ...
  const applyNest = (nest) => {
    ...
    setNestRampCurve(nest.rampCurve != null ? nest.rampCurve : null);
    setNestTshWidthIn(nest.tshWidthIn != null ? nest.tshWidthIn : null);
    setNestWallThicknessIn(nest.wallThicknessIn != null ? nest.wallThicknessIn : null);
    setNestBacksetIn(nest.backsetIn != null ? nest.backsetIn : null);
    setNestTipFlatIn(nest.tipFlatIn != null ? nest.tipFlatIn : null);
    setNestChimneyIn(nest.chimneyIn != null ? nest.chimneyIn : null);
    setNestTipHeightIn(nest.tipHeightIn != null ? nest.tipHeightIn : null);
    setNestFlueLengthIn(nest.flueLengthIn != null ? nest.flueLengthIn : null);
  };
  const clearNest = () => {
    setNestName(null); setNestRampAngleDeg(null); setNestFlueDepthIn(null);
    setNestTshLengthIn(null); setNestFippleAngleDeg(0); setNestRampCurve(null);
    setNestTshWidthIn(null); setNestWallThicknessIn(null); setNestBacksetIn(null);
  ...
  const doSaveNest = () => {
    const entry = saveNestToLibrary(nestSaveName, {
      ...
      rampCurve: nestRampCurve != null ? nestRampCurve : 0,
      tshWidthIn: nestTshWidthIn != null ? nestTshWidthIn : parseFloat(shW),
      wallThicknessIn: nestWallThicknessIn != null ? nestWallThicknessIn : Math.max(0.05, (bore / 2) * 0.28),
      backsetIn: nestBacksetIn != null ? nestBacksetIn : 0,
      tipFlatIn: nestTipFlatIn != null ? nestTipFlatIn : 0.01,
      chimneyIn: nestChimneyIn != null ? nestChimneyIn : 0,
      tipHeightIn: nestTipHeightIn != null ? nestTipHeightIn : 0.0078,
      flueLengthIn: nestFlueLengthIn,
    });
    if (entry) {
  ...
  const chambersForDiagram = [{
    ...
    shL: (nestTshLengthIn != null ? nestTshLengthIn : shL),
  ...
  const flowSnapshotJson = JSON.stringify({
    ...
    chimneyIn: nestChimneyIn != null ? nestChimneyIn : 0,
    tipHeightIn: nestTipHeightIn != null ? nestTipHeightIn : 0.0078,
    flueLengthIn: nestFlueLengthIn != null ? nestFlueLengthIn : null,
    holeCount,
    holes: (holes || []).map(h => ({
  ...
  return (
    <div style={{maxWidth:740,margin:"0 auto"}}>
      ...
      {fluteStyle==="drone" && (<>
        ...
        {/* Drone body construction: distinct pipes vs one solid block */}
        <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center"}}>
          <span style={{fontSize:12,color:muted,flexShrink:0}}>Body:</span>
          <button onClick={()=>setDroneBody("separate")} style={{...pill(droneBody==="separate"),flex:1,padding:"8px"}}>
            Separate pipes
          </button>
          <button onClick={()=>setDroneBody("solid")} style={{...pill(droneBody==="solid"),flex:1,padding:"8px"}}>
            One solid body
          </button>
        </div>
...
const FLOW_DEFAULT_DESIGN = {
  ...
  rampAngleDeg: 30, fippleAngleDeg: 35, rampCurve: 0,
  breathHoleWidthIn: 0.28, breathHoleLengthIn: 0.53,
  wallThicknessIn: 0.0875, backsetIn: 0, tipFlatIn: 0.01, chimneyIn: 0,
...
function computeFluteAeroacoustics(design, pressurePa) {
  ...
  if (d.holes && d.holes.length && d.L > 0) {
    ...
    if (fromTSHs.length) {
      ...
      // include the open-end corrections (0.61·a each side): on large
      // bores they're a big fraction of the short top-note column, and
      // omitting them wildly overestimated fTop (θtop pinned at 0 for
      // e.g. a 2" bore C4).
      const dLc = 2 * 0.61 * (d.bore / 2);
      fTop = f0 * ((d.L + dLc) / (minFromTSH + dLc));
      thetaTop = U / (fTop * lc);
    }
  }
...
function scoreFlowQuality(m) {
  ...
  // lower falloff starts at 1 (not 2): below ~4-5 the note is weak and
  // below ~2 it won't reliably speak, but a hard cliff to 0 gave no
  // gradient at all for big-bore low flutes — the score should still
  // respond as settings improve.
  const driveRoot = band(m.theta,    1, 5, 10, 16);
  const driveTop  = band(m.thetaTop, 1, 5, 10, 16);
...
class FlowStudioViewer {
  ...
  computeLayout() {
    ...
    const T    = (params.wallMm > 0)
      ? Math.max(1, Math.min(params.wallMm, 12))             // user wall thickness
      : Math.max(3, Math.min(6, B * 0.20));                  // auto from bore
    ...
    const flueLen = (params.flueLenMm > 0)
      ? Math.max(3, Math.min(params.flueLenMm, 55))          // user flue length
      : Math.max(8, design.shW * 2 * 25.4);                  // Prairie default: 2 × width
    ...
    const footX = lc + Lmm;
    // Breath hole (mouth end): width = the opening's height in this
    // cutaway, length = the mouthpiece passage the player blows through.
    const bhW = Math.max(2, (design.breathHoleWidthIn || 0.28) * 25.4);
    const bhL = Math.max(2, (design.breathHoleLengthIn || 0.53) * 25.4);
    const sacMidY = (yBoreBot + yWallBot) / 2;
    // Prairie extras (all user-adjustable): backset (0–D/3, the bore
    // extending back under the flue), the ~1/100" tip flat, and the bird
    // chimney height over the TSH.
    const bk = Math.max(0, Math.min(params.backsetMm || 0, B / 3, flueLen * 0.6));
    const tipFlat = Math.max(0, Math.min(params.tipFlatMm || 0, 2));
    const chim = Math.max(0, Math.min(params.chimneyMm || 0, 12));
    // Fipple tip height ABOVE the flue floor (Prairie 1/128" default)
    const tipHeight = Math.max(0, Math.min(params.tipHeightMm || 0.2, h * 0.9));
    return { B, sacL, Lmm, T, h, lc, Wd, flueLen, rampRad, rampRun,
             ...
             bhW, bhL, sacMidY, bk, tipFlat, chim, tipHeight };
  }
  ...
  buildChamber() {
    ...
    // HALF-SECTION cutaway: the solid is only the BACK half of the flue's
    // width (z ∈ [-Wd/2, 0]); the airflow occupies the FRONT half of the
    // chamber volume (z ∈ (0, +Wd/2]) — INSIDE the wind chamber, bounded
    ...
    {
      const k = Math.max(0, Math.min(1, params.rampCurve || 0));
      const bs = new THREE.Shape();
      bs.moveTo(L.rampStartX, L.yBoreBot);
      if (k > 0.01) {
        // concave (curving-down) ramp face — control point pulled toward
        // the base-side corner; 0 = straight, 1 = full scoop
        const mx = (L.rampStartX + L.rampTopX) / 2, my = (L.yBoreBot + L.yFlueFloor) / 2;
        bs.quadraticCurveTo(mx + k * (L.rampTopX - mx), my + k * (L.yBoreBot - my), L.rampTopX, L.yFlueFloor);
      } else {
        bs.lineTo(L.rampTopX, L.yFlueFloor);
      }
    ...
    // SAC ceiling (top wall): its surface-level edge stops one TSH-length
    // before the bird (the exit opening), and its end face is angled DOWN
    // INTO the SAC at the ramp angle (Prairie's "smooth transitions") —
    // the underside extends upstream so air sweeps up through the opening.
    const ceilRun = L.T / Math.tan(L.rampRad);
    addShape([
      [L.sacX0 - L.T, L.yWallBot],
      [L.exitX0 - ceilRun, L.yWallBot],
      [L.exitX0, L.yFloorTop],
      [L.sacX0 - L.T, L.yFloorTop],
    ], wood);

    // Mouth end: mouthpiece cap with the BREATH HOLE — the opening the
    // player blows through into the SAC (its width and passage length come
    // straight from the design bridge). Two blocks above/below the hole.
    box(L.sacX0 - L.bhL, L.sacX0, L.yBoreBot, L.sacMidY - L.bhW / 2, wood);
    box(L.sacX0 - L.bhL, L.sacX0, L.sacMidY + L.bhW / 2, L.yWallBot, wood);
    ...
    if (L.chim > 0.1) {
      // Prairie's bird CHIMNEY: the bird extends over the TSH, its
      // underside raised `chim` above the surface — a shaft above the
      // window (a deep chimney flattens the pitch).
      box(0, L.lc + 2, L.yFloorTop + L.chim, L.birdTop, woodDim);
    }
  ...
  // Crisp radial sprite for tracer mode.
  makeSpriteTexture() {
  ...
  initParticleFlow() {
    ...
    if (!this.smokeTex) this.smokeTex = this.makeSmokeTexture();
  ...
  // Bright additive tracer points vs soft translucent smoke — sizes and
  // alphas are computed per particle in the update loop; here we switch
  // the sprite texture and blend mode.
  applyParticleStyle() {
    if (!this.particleSystem) return;
    const m = this.particleSystem.material;
    m.uniforms.map.value = this.smoke ? this.smokeTex : this.spriteTex;
    m.blending = this.smoke ? THREE.NormalBlending : THREE.AdditiveBlending;
    m.needsUpdate = true;
  }
  ...
  // Path stations shared by the particle sim and the advected vector
  // field, so arrows travel exactly the same journey as the particles.
  pathStations() {
    const L = this.layout;
    const x0 = L.sacX0, x1 = L.rampStartX, x2 = L.rampTopX, x3 = 0, x4 = L.lc;
    const x5 = L.footX;
    const x6 = L.footX + Math.max(25, L.B * 2.2);
    const total = (x6 - x0);
    const spd = (x) =>
      x < x1 ? 0.12 : x < x2 ? 0.5 : x < x3 ? 1.0 : x < x4 ? 0.4 : x < x5 ? 0.25 : 0.16;
    return { x0, x1, x2, x3, x4, x5, x6, total, spd };
  }

  // ── Weather-style flow vector field ────────────────────────────────
  // Instanced arrow darts sampled along the whole air path, oriented by
  // local flow direction and colored/scaled by relative speed (blue → red,
  // like a wind map). The darts inside the TSH window flap with the jet.
  initVectorField() {
    const geo = new THREE.ConeGeometry(0.42, 1, 6);
    geo.translate(0, 0.5, 0); // base at origin, pointing +Y (unit length)
    this.vecMax = 90;
    ...
    this.vecMesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    this.vecMesh.visible = this.getState().showVectors !== false;
  ...
  updateVectorField(t, dt) {
    ...
    if (this.vecMesh.instanceColor) this.vecMesh.instanceColor.needsUpdate = true;
  }
  ...
  dispose() {
    ...
    if (this.smokeTex) this.smokeTex.dispose();
    if (this.vecMesh) {
      this.vecMesh.geometry.dispose();
      this.vecMesh.material.dispose();
    }
...
function FlowStudioPage() {
  ...
  const [params, setParams] = useState(() => {
    ...
    return {
      ...
      rampCurve: Math.max(0, Math.min(1, d.rampCurve || 0)),
      tshWidthMm: +(d.shW * 25.4).toFixed(1),
      wallMm: +(((d.wallThicknessIn > 0 ? d.wallThicknessIn : (d.bore / 2) * 0.28)) * 25.4).toFixed(1),
      backsetMm: +((d.backsetIn || 0) * 25.4).toFixed(1),
      tipFlatMm: +((d.tipFlatIn != null ? d.tipFlatIn : 0.01) * 25.4).toFixed(2),
      chimneyMm: +((d.chimneyIn || 0) * 25.4).toFixed(1),
      tipHeightMm: +((d.tipHeightIn != null ? d.tipHeightIn : 1 / 128) * 25.4).toFixed(2),
      flueLenMm: +(((d.flueLengthIn > 0 ? d.flueLengthIn : d.shW * 2)) * 25.4).toFixed(1),
    };
  });
  ...
  const designEff = useMemo(() => ({
    ...
    shW: params.tshWidthMm > 0 ? params.tshWidthMm / 25.4 : design.shW,
    wallThicknessIn: params.wallMm > 0 ? params.wallMm / 25.4 : design.wallThicknessIn,
    backsetIn: (params.backsetMm || 0) / 25.4,
    tipFlatIn: (params.tipFlatMm || 0) / 25.4,
    chimneyIn: (params.chimneyMm || 0) / 25.4,
    tipHeightIn: (params.tipHeightMm || 0.2) / 25.4,
    flueLengthIn: params.flueLenMm > 0 ? params.flueLenMm / 25.4 : design.flueLengthIn,
  }), [design, params]);
  ...
  useEffect(() => {
    if (viewerRef.current) viewerRef.current.setVectorsVisible(showVectors);
  }, [showVectors]);
  ...
  const doSaveNest = () => {
    const entry = saveNestToLibrary(saveName || `Flow Studio ${design.keyName || "Nest"}`, {
      ...
      fippleAngleDeg: params.fippleAngle,
      rampCurve: params.rampCurve || 0,
      tshWidthIn: +((params.tshWidthMm || 8) / 25.4).toFixed(4),
      wallThicknessIn: params.wallMm > 0 ? +(params.wallMm / 25.4).toFixed(4) : null,
      backsetIn: +((params.backsetMm || 0) / 25.4).toFixed(4),
      tipFlatIn: +((params.tipFlatMm || 0) / 25.4).toFixed(4),
      chimneyIn: +((params.chimneyMm || 0) / 25.4).toFixed(4),
      tipHeightIn: +((params.tipHeightMm || 0.2) / 25.4).toFixed(4),
      flueLengthIn: params.flueLenMm > 0 ? +(params.flueLenMm / 25.4).toFixed(4) : null,
    });
    if (entry) {
  ...
  const applyNest = (id) => {
    ...
    setParams(p => ({
      ...
      rampCurve: item.rampCurve != null ? Math.max(0, Math.min(1, item.rampCurve)) : p.rampCurve,
      tshWidthMm: item.tshWidthIn != null ? +(item.tshWidthIn * 25.4).toFixed(1) : p.tshWidthMm,
      wallMm: item.wallThicknessIn != null ? +(item.wallThicknessIn * 25.4).toFixed(1) : p.wallMm,
      backsetMm: item.backsetIn != null ? +(item.backsetIn * 25.4).toFixed(1) : p.backsetMm,
      tipFlatMm: item.tipFlatIn != null ? +(item.tipFlatIn * 25.4).toFixed(2) : p.tipFlatMm,
      chimneyMm: item.chimneyIn != null ? +(item.chimneyIn * 25.4).toFixed(1) : p.chimneyMm,
      tipHeightMm: item.tipHeightIn != null ? +(item.tipHeightIn * 25.4).toFixed(2) : p.tipHeightMm,
      flueLenMm: item.flueLengthIn != null && item.flueLengthIn > 0 ? +(item.flueLengthIn * 25.4).toFixed(1) : p.flueLenMm,
    }));
  ...
  return (
    <div style={S.page}>
      <div style={S.canvasWrap}>
        ...
        {showVectors && (
          <div style={{ position: "absolute", bottom: 10, right: 10, background: "rgba(2,6,23,0.8)", padding: "6px 10px", borderRadius: 6, border: "1px solid #1e293b" }}>
            <div style={{ fontSize: 9, color: "#94a3b8", marginBottom: 3, fontWeight: 700, letterSpacing: "0.05em" }}>FLOW SPEED</div>
            <div style={{ width: 110, height: 8, borderRadius: 4, background: "linear-gradient(90deg, hsl(223,100%,55%), hsl(168,100%,55%), hsl(112,100%,55%), hsl(56,100%,55%), hsl(0,100%,55%))" }} />
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#64748b", marginTop: 2 }}>
              <span>0</span><span>{physics.U.toFixed(0)} m/s</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={S.side}>
        ...
        <div style={S.card}>
          ...
          <div style={S.row}><span style={S.k}>Breath hole</span><span style={S.v}>{(design.breathHoleWidthIn || 0).toFixed(2)}&Prime; &times; {(design.breathHoleLengthIn || 0).toFixed(2)}&Prime;</span></div>
        ...
        <div style={S.card}>
          ...
          <input type="range" min={5} max={55} step={1} value={params.fippleAngle}
            onChange={e => setParam("fippleAngle", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Ramp curve</span><b>{Math.round((params.rampCurve || 0) * 100)}%</b></div>
          <input type="range" min={0} max={100} step={5} value={Math.round((params.rampCurve || 0) * 100)}
            onChange={e => setParam("rampCurve", (+e.target.value) / 100)} style={S.slider} />
          <div style={S.sLabel}><span>TSH / flue width</span><b>{(params.tshWidthMm || 0).toFixed(1)} mm</b></div>
          <input type="range" min={4} max={16} step={0.1} value={params.tshWidthMm || 8}
            onChange={e => setParam("tshWidthMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Wall thickness</span><b>{(params.wallMm || 0).toFixed(1)} mm</b></div>
          <input type="range" min={1.5} max={9.5} step={0.1} value={params.wallMm || 4}
            onChange={e => setParam("wallMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Backset (0–D/3)</span><b>{(params.backsetMm || 0).toFixed(1)} mm</b></div>
          <input type="range" min={0} max={8} step={0.1} value={params.backsetMm || 0}
            onChange={e => setParam("backsetMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Tip flat</span><b>{(params.tipFlatMm || 0).toFixed(2)} mm</b></div>
          <input type="range" min={0} max={1} step={0.05} value={params.tipFlatMm || 0}
            onChange={e => setParam("tipFlatMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Bird chimney</span><b>{(params.chimneyMm || 0).toFixed(1)} mm</b></div>
          <input type="range" min={0} max={8} step={0.25} value={params.chimneyMm || 0}
            onChange={e => setParam("chimneyMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Flue length</span><b>{(params.flueLenMm || 0).toFixed(1)} mm</b></div>
          <input type="range" min={5} max={45} step={0.5} value={params.flueLenMm || 16}
            onChange={e => setParam("flueLenMm", +e.target.value)} style={S.slider} />
          <div style={S.sLabel}><span>Tip height (above flue floor)</span><b>{(params.tipHeightMm || 0).toFixed(2)} mm</b></div>
          <input type="range" min={0} max={1} step={0.02} value={params.tipHeightMm || 0.2}
